...
void microcheck()
{
    ...
    if (uc == CSettings::uCode_NotFound)
    {
        if (g_ucode_error_report)
        {
            ...
            // Megaton Hammer headless: skip the modal dialog (it blocks automation); the fallback ucode
            // is used regardless, and headless runs don't need correct graphics. The trace above records it.
