...
{
    // Megaton Hammer playtest (MH_HEADLESS / MH_INTERP): never pop a BLOCKING modal — it freezes the
    // playtest. The most common one is the RSP plugin's benign "uCode crc not found in INI, using
    // currently selected uCode" warning on a debug ROM whose microcode CRC isn't in PJ64's database
    // (the default F3DEX2 uCode renders fine). Log and continue instead.
    if (getenv("MH_HEADLESS") != nullptr || getenv("MH_INTERP") != nullptr)
    {
        WriteTrace(TraceUserInterface, TraceWarning, "[MH] suppressed warning modal: %s", Message);
        return;
    }
    HWND Parent = nullptr;
    if (m_hWnd)
...
void CNotificationImp::DisplayError(const char * Message) const
{
    ...
    // Megaton Hammer playtest: don't block on a modal error dialog (e.g. a transient EmulationStarting
    // exception during bring-up of an injected debug ROM). It's already logged above (and crashes go to
    // the MH playtest log via MegatonHammer_LogCrash); a frozen window reads as a "crash" to the user.
    if (getenv("MH_HEADLESS") != nullptr || getenv("MH_INTERP") != nullptr)
    {
        return;
    }
