...
// Megaton Hammer: per-frame playtest hook (observe SaveContext + drive warp on
// the editor-injected OoT MQ debug ROM). Defined in MegatonHammer.cpp.
...
bool CN64System::EmulationStarting(CThread * thread)
{
    ...
    if (g_BaseSystem->SetActiveSystem(true))
    {
        ...
        {
            // Megaton Hammer: record the crash CPU state (PC/EPC/BadVAddr) to the playtest log so a crash
            // — e.g. grabbing a ledge in an injected scene — is diagnosable against the decomp map.
            MegatonHammer_LogCrash();
            // Megaton Hammer headless: a modal error dialog blocks unattended automation (and a crash on
            // a custom/MM debug ROM is expected during bring-up). Log it instead of popping a dialog.
            if (getenv("MH_HEADLESS") != nullptr)
            {
                WriteTrace(TraceN64System, TraceError, "%s: Exception caught File: %s Line: %d", __FUNCTION__, __FILE__, __LINE__);
            }
            else
            {
...
void CN64System::RefreshScreen()
{
    ...
    // Megaton Hammer headless verification (MH_HEADLESS): skip the video present (software-GL swap)
    // to keep the CPU running fast for RAM-only checks. Cached once; env is set before launch.
    static int mhHeadless = (getenv("MH_HEADLESS") != nullptr) ? 1 : 0;
    if (!mhHeadless)
