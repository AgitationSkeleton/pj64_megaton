...
bool CRSP_Plugin::Initiate(CPlugins * Plugins, CN64System * System)
{
    ...
    if (m_PluginInfo.Version >= 0x103)
    {
        ...
        else
        {
            ...
            // Megaton Hammer headless verification: when MH_HEADLESS is set, skip graphics-task
            // processing (software OpenGL is the boot bottleneck in a GPU-less session). The RSP
            // plugin still signals task completion, so the CPU boots/warps fast for RAM-only checks.
